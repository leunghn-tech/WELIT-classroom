...
function Header({ onHome, action, onOpenQuestionManager, onOpenClassroomToolkit }) {
  return <header className="topbar"><Brand /><div className="topbar-right"><span className="demo-pill">課堂試玩・可儲存進度</span><ExamTimer /><TeacherFeedbackSettings onOpenQuestionManager={onOpenQuestionManager} onOpenClassroomToolkit={onOpenClassroomToolkit} />{action}{onHome && <button className="icon-button" onClick={onHome} aria-label="返回首頁"><LayoutDashboard size={20} /></button>}</div></header>;
}

function TeacherSettingsDock({ onOpenQuestionManager, onOpenClassroomToolkit }) {
  return <div className="teacher-settings-access"><TeacherFeedbackSettings onOpenQuestionManager={onOpenQuestionManager} onOpenClassroomToolkit={onOpenClassroomToolkit} /></div>;
}

function Home({ onStart, onCatalog, onOpenQuestionManager, onOpenClassroomToolkit }) {
...
export default function App() {
  ...
  const [screen, setScreen] = useState(previewUnit ? 'activity' : previewTopic ? 'demo' : params.get('view') === 'classroom-toolkit' ? 'classroom-toolkit' : params.get('view') === 'english-catalog' || params.get('view') === 'math-catalog' || chineseCatalogPreview ? 'catalog' : params.get('view') === 'courses' ? 'courses' : 'home');
  ...
  if (screen === 'activity' && activeUnit) {
    ...
    const withTeacherControls = (content) => <>{content}<TeacherActivityControls session={classroomSession} activeUnit={activeUnit} onBack={backToCatalog} onOpenToolkit={() => openClassroomToolkit('activity')} onNextPlanned={() => startNextClassroomUnit(activeUnit)} /></>;
    ...
    if (activeUnit.activityMode === 'team-battle' && activeUnit.id.includes('-MATH-')) return withTeacherControls(<><TeamMonsterMathActivity unit={activeUnit} onBack={backToCatalog} onComplete={markUnitCompleted} /><TeacherSettingsDock onOpenClassroomToolkit={() => openClassroomToolkit('activity')} /></>);
    ...
    if (activeUnit.id.includes('-MATH-')) return withTeacherControls(<MathActivity unit={activeUnit} onBack={backToCatalog} onComplete={markUnitCompleted} />);
  ...
  if (screen === 'courses') return <Courses onBack={() => setScreen('home')} onOpen={openDemo} onCatalog={openCatalog} onOpenQuestionManager={() => openQuestionManager('courses')} onOpenClassroomToolkit={() => openClassroomToolkit('courses')} />;
  if (screen === 'demo' && topic) return <Demo topic={topic} onBack={() => setScreen('courses')} />;
  return <Home onStart={() => setScreen('courses')} onCatalog={openCatalog} onOpenQuestionManager={() => openQuestionManager('home')} onOpenClassroomToolkit={() => openClassroomToolkit('home')} />;
}
