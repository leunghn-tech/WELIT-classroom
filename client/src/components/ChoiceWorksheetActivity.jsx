...
export default function ChoiceWorksheetActivity({ unit, onBack, onComplete }) {
  ...
  return <main className="site-shell choice-worksheet-page"><WorkbenchFrame unit={unit} taskLabel={`任務 ${questionIndex + 1} / ${questions.length}`} /><header className="match-topbar"><button onClick={onBack} className="match-back">返回中文目錄</button><div><span>{unit.area}・{unit.title}</span><b>第 {questionIndex + 1} / {questions.length} 題</b></div><div className="match-progress"><i style={{ width: `${((questionIndex + 1) / questions.length) * 100}%` }} /></div></header><section className="choice-worksheet-stage"><div className="match-heading"><span><Sparkles size={16} /> {label.name}</span><h1>{question.prompt}</h1><p>老師提示：{label.tip}</p></div><section className="context-paper"><div className="context-paper-head"><span>{label.sheet}</span><small>{label.prompt}</small></div><p>{question.context}</p>{unit.interaction === 'context-choice' && <strong>要推測的字詞：<em>{question.target}</em></strong>}{unit.interaction === 'connector-cloze' && <div className="connector-sentence"><SentenceWithBlank text={question.sentence} /></div>}</section><section className="worksheet-options"><div className="bank-title"><span>選擇答案</span><small>每次重玩會重新排列</small></div><div data-option-safety-grid="true" data-question-id={question.id} data-answer-value={String(question.answer)}>{choices.map((choice, index) => <button key={choice} data-choice-value={String(choice)} disabled={Boolean(feedback)} onClick={() => answer(choice)} className={`${selected === choice ? feedback?.correct ? 'selected-correct' : 'selected-wrong' : ''}`}><span>{String.fromCharCode(65 + index)}</span><b>{choice}</b></button>)}</div></section>{feedback && <div className={`worksheet-feedback ${feedback.correct ? 'correct' : 'incorrect'}`}>{feedback.correct ? <><Check size={20} /><div><b>答對了！</b><p>{question.explanation}</p></div></> : <><X size={20} /><div><b>這個答案未符合句子線索。</b><p>請再讀一次情境，或選擇重新作答。</p></div></>}<div className="complete-actions">{feedback.correct ? <button onClick={nextQuestion}>{questionIndex === questions.length - 1 ? '查看結算' : '下一題'} <ChevronRight size={17} /></button> : <button onClick={resetQuestion}><RotateCcw size={16} /> 重新作答</button>}<button onClick={onBack}>返回題目板</button></div></div>}</section></main>;
}
