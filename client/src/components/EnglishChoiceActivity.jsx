...
export default function EnglishChoiceActivity({ unit, onBack, onComplete }) {
  ...
  return <main className="site-shell english-choice-page"><ActivityFrame unit={unit} taskLabel={`任務 ${questionIndex + 1} / ${questions.length}`} /><header className="match-topbar english-match-topbar"><button onClick={onBack} className="match-back">返回英文目錄</button><div><span>{unit.area}・{unit.title}</span><b>第 {questionIndex + 1} / {questions.length} 題</b></div><div className="match-progress" aria-label={`進度 ${questionIndex + 1} / ${questions.length}`}><i style={{ width: `${((questionIndex + 1) / questions.length) * 100}%` }} /></div></header><section className="english-activity-stage"><div className="match-heading"><span><Sparkles size={16} /> {label.name}</span><h1>{question.prompt}</h1><p>老師提示：{label.tip}</p></div><section className="english-worksheet"><MaterialCard question={question} interaction={unit.interaction} label={label} /><section className="english-answer-zone"><div className="bank-title"><span>選擇答案</span><small>每次開始會重新排列</small></div><div className="english-option-grid">{choices.map((choice, index) => <button key={choice} disabled={Boolean(feedback)} onClick={() => answer(choice)} className={selected === choice ? feedback?.correct ? 'selected-correct' : 'selected-wrong' : ''}><span>{String.fromCharCode(65 + index)}</span><b>{choice}</b></button>)}</div></section></section>{feedback && <section className={`english-feedback ${feedback.correct ? 'correct' : 'incorrect'}`} role="status"><div className="english-feedback-icon">{feedback.correct ? <Check size={22} /> : <X size={22} />}</div><div><b>{feedback.correct ? '答對了！' : '這次還未選中正確答案。'}</b><p>{feedback.correct ? question.explanation : <>正確答案是 <strong>{question.answer}</strong>。{question.explanation}</>}</p><div className="complete-actions">{feedback.correct ? <button onClick={next}>{questionIndex === questions.length - 1 ? '查看結算' : '下一題'} <ChevronRight size={17} /></button> : <button onClick={retry}><RotateCcw size={16} /> 依提示再試</button>}<button onClick={onBack}>返回英文目錄</button></div></div></section>}</section></main>;
}
