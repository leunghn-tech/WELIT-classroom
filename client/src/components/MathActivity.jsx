...
export default function MathActivity({ unit: incomingUnit, onBack, onComplete }) {
  ...
  const activityMaterial = unit.interaction === 'math-life-application' ? <LifeApplicationModel question={question} /> : unit.interaction === 'math-number-line' ? <NumberLine line={question.line} selected={selected} disabled={Boolean(feedback)} onSelect={answer} /> : unit.interaction === 'math-ten-frame' ? <TenFrame frame={question.frame} selectedFill={frameFill} onToggle={toggleFrame} disabled={Boolean(feedback)} /> : additionParts ? <AdditionCollectionModel key={`${question.id}-${shuffleRound}`} parts={additionParts} disabled={Boolean(feedback)} onCompletionChange={setModelComplete} /> : coinValues ? <CoinCollectionModel key={`${question.id}-${shuffleRound}`} coins={coinValues} disabled={Boolean(feedback)} onCompletionChange={setModelComplete} /> : question.visual ? <MathPicture visual={question.visual} /> : unit.interaction === 'math-choice' && hasChoiceModel ? <ChoiceMathModel question={question} groups={activeGroups} onGroupsChange={setModelGroups} onGroupingFeedback={playModelSound} disabled={Boolean(feedback)} /> : null;
  return <main className="site-shell math-activity-page"><MathFrame unit={unit} questionLabel={`第 ${questionIndex + 1}／${questions.length}`} /><nav className="math-activity-controls"><button onClick={onBack} className="match-back">返回數學目錄</button><div className="math-progress-readout"><b>第 {questionIndex + 1} 題</b><small>共 {questions.length} 題</small></div><div className="match-progress" aria-label={`進度 ${questionIndex + 1} / ${questions.length}`}><i style={{ width: `${((questionIndex + 1) / questions.length) * 100}%` }} /></div></nav><section className="math-activity-stage"><div className="math-heading"><span><Sparkles size={16} /> {visualType}</span><h1><FormalMathText text={question.prompt} /></h1><p>老師提示：{tips[unit.interaction]}</p></div><MathKnownData question={question} />{activityMaterial ? <section className={`math-worksheet ${question.visual || isFrameAdd || unit.interaction === 'math-number-line' || unit.interaction === 'math-choice' ? 'has-material' : 'answer-only'}`}>{activityMaterial}{isFrameAdd && !feedback ? <button className="math-frame-submit" onClick={() => answer(frameFill.size)}>以十格框作答 <ChevronRight size={17} /></button> : null}{answerLocked && !feedback ? <div className="math-answer-locked"><span>✦</span> {groupingCount ? '先完成兩個一組的分組，再開啟答案紙條。' : '先完成操作模型，再開啟答案紙條。'}</div> : null}</section> : null}{(!isFrameAdd || feedback) && unit.interaction !== 'math-number-line' && choices.length && !answerLocked ? <section className="math-answer-zone"><div className="bank-title"><span>選擇答案</span><small>每次開始會重新排列</small></div><div className="math-option-grid">{choices.map((choice, index) => <button key={String(choice)} disabled={Boolean(feedback)} onClick={() => answer(choice)} className={selected === choice ? feedback?.correct ? 'selected-correct' : 'selected-wrong' : ''}><span>{String.fromCharCode(65 + index)}</span><b><FormalMathText text={choice} /></b></button>)}</div></section> : null}{feedback && <section className={`math-feedback ${feedback.correct ? 'correct' : 'incorrect'}`} role="status"><div>{feedback.correct ? <Check size={22} /> : <X size={22} />}</div><section><b>{feedback.correct ? '答對了！' : '這次還未選中正確答案。'}</b><p>{feedback.correct ? <FormalMathText text={question.explanation} /> : <>正確答案是 <strong><FormalMathText text={question.answer} /></strong>。<FormalMathText text={question.explanation} /></>}</p>{unit.examMode ? <HighMathWorkedSteps unit={unit} question={question} /> : null}<div className="complete-actions">{feedback.correct ? <button onClick={next}>{questionIndex === questions.length - 1 ? '查看結算' : '下一題'} <ChevronRight size={17} /></button> : <button onClick={retry}><RotateCcw size={16} /> 依提示再試</button>}<button onClick={onBack}><ArrowLeft size={16} /> 返回數學目錄</button></div></section></section>}</section></main>;
}
