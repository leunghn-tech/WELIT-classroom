...
function MathFrame({ unit, questionLabel, showMathWorkedHints = false }) {
  ...
  return <><header className="activity-workbench-frame math-activity-frame"><span className="activity-file-tab">{grade}<br />MATH</span><div className="activity-brand-lockup"><span className="activity-brand-mark"><i></i><i></i><i></i><Sparkles size={18} /></span><div><b>Edu<span>Quest</span></b><small>數學課堂檔案</small></div></div><div className="activity-course-file"><span>{gradeLabel(grade)}數學 · 操作工作紙</span><b>{unit.title}</b>{questionLabel ? <small className="math-file-status">{questionLabel}　·　看模型 → 作答</small> : null}<i className="math-file-stamp">✦ EDUQUEST · MATH FILE</i></div>{questionLabel && <ExamTimer />}</header>{questionLabel && showMathWorkedHints ? <HintSatchel title={unit.hintTitle || '數學解題錦囊'} hint={unit.hint || tips[unit.interaction]} steps={unit.hintSteps || []} /> : null}</>;
}
