...
function MathKnownData({ question }) {
  ...
  const values = [...new Set((question.prompt.match(/\d+(?:\.\d+)?/g) || []))];
  ...
  const known = explicitKnown.length ? explicitKnown : [...visualKnown, ...(values.length ? [`題目中的數字：${values.join('、')}`] : []), ...(visualKnown.length || values.length ? ['先分辨題目要你求甚麼。'] : ['先找出題目提供的資料。', '再圈出要回答的量與單位。'])];
  return <aside className="math-known-data" aria-label="題目已知資料"><header><span>已知資料</span><small>先圈重點，再作答</small></header><ul>{known.map((item) => <li key={item}>{item}</li>)}</ul></aside>;
}
