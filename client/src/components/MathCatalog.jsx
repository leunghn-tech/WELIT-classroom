...
function UnitCard({ unit, completedUnits, classroomMode, examMode, onStart, featured = false }) {
  ...
  return <button className={`math-unit-card ${featured ? 'life-application-card' : ''}`} onClick={() => onStart(unit)}><div><span>{featured ? '小六整合題組' : classroomMode === 'team-battle' ? '全班合力題組' : examMode ? '呈分試題組' : interactionLabel}</span><b>{unit.questions.length} 題</b></div><h3>{unit.title}</h3><p>{unit.objective}</p><footer><small>{completed ? `${completed} 題已完成` : featured ? '開啟分步解題' : classroomMode === 'team-battle' ? '全班合力開戰' : '開始練習'} </small><i><b style={{ width: `${percent}%` }} /></i><ChevronRight size={17} /></footer></button>;
}
