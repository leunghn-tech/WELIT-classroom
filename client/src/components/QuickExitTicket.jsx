...
function QuestionContext({ question }) {
  ...
  return context ? <p className="exit-question-context">{context}</p> : null;
}
...
export default function QuickExitTicket({ questionBanks, onBack }) {
  ...
  if (showDistribution) return <main className={`site-shell exit-ticket-page ${subjectClass}`}><header className="exit-ticket-header"><span><BarChart3 size={18} /> 快速出口題・答案分布</span><button onClick={onBack}><ArrowLeft size={16} /> 返回課程</button></header><section className="exit-ticket-summary"><span><Trophy size={24} /> 課末診斷完成</span><h1>{selectedUnit?.title}</h1><p>共抽取 {questions.length} 題，老師共記錄 {totalVotes} 張舉手票。學科色柱為正確答案，其他柱可協助找出需重教的概念。</p><div className="exit-distribution-list">{questions.map((item, itemIndex) => <article key={item.id}><header><span>第 {itemIndex + 1} 題</span><b>{item.prompt}</b></header><QuestionContext question={item} />{item.choices.map((choice) => { const votesForChoice = votes[item.id]?.[choice] || 0; const percent = totalVotes ? Math.round((votesForChoice / Math.max(1, Object.values(votes[item.id] || {}).reduce((sum, value) => sum + value, 0))) * 100) : 0; return <div className={choice === item.answer ? 'correct-answer' : ''} key={String(choice)}><span>{choice}</span><i><b style={{ width: `${percent}%` }} /></i><small>{votesForChoice} 票 {choice === item.answer ? '・正確答案' : ''}</small></div>; })}</article>)}</div><div className="exit-summary-actions"><button onClick={() => draw(count)}><RotateCcw size={16} /> 再抽一組</button><button onClick={onBack}><ArrowLeft size={16} /> 返回課程</button></div></section></main>;
